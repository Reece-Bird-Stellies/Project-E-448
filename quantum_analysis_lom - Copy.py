...
# =========================
def _extract_hbar_w_r_and_n_phi(Lj, Csigma, Nmax):
    """
    ...
    hbar_w_r, U     = np.linalg.eigh(H)     # E = ℏ_ω, therefor the diagonal matrix is ℏ_ω_r
    n_phi           = U.conj().T @ n_charge @ U
    
    return (hbar_w_r, n_phi)
...
def compute_hamiltonian_lom(Lj, Lr, Cj, Cs, Cg, Cr, Nc, Nq, Nmax):
    ...
    (hbar_w_r, n_phi) = _extract_hbar_w_r_and_n_phi(Lj, Csigma, Nmax)
    ...
    proj_terms = []
    for j in range(Nq):
        x = hbar_w_r[j] * np.kron(np.outer(Iq_hat[j], Iq_hat[j]), Ic_hat)   #ℏ already included in ℏ_ω_r
        proj_terms.append(x)
